...
import { removeConversation, deleteChat } from '../../store/UserStore';
import MediaFilesModal from '../ThemeChat/FilesThemes/MediaFilesModal';
import Media from '../ThemeChat/FilesThemes/Media';
import AudioMessageItem from '../../components/AudioMessageChat/AudioMessageItem ';
...
const Modal = ({ otherUserId }) => {
    ...
    const confirmDelete = async (e) => {
        ...
        if (otherUserId) {
            try {
                await dispatch(deleteChat(otherUserId));
                dispatch(removeConversation(otherUserId));
                closeModal();
            } catch (error) {
    ...
    return (
        ...
        {isModalOpen && (
          ...
          >
                ...
                {showDeleteConfirm ? (
                  <div className="p-4 sm:p-6">
                    <div className="text-center">
                      <p className="text-gray-800 mb-4 sm:mb-6 text-sm sm:text-base">Вы уверены, что хотите очистить историю чата?</p>
                  ...
                  <ul className='list-none m-0 p-0'>
                    ...
                    <li className="border-b border-gray-100">
                      <button
                        ...
                        className="bg-[rgba(1,14,24,0.946)] w-full px-4 sm:px-5 py-3 sm:py-4 border-none text-left cursor-pointer text-xs sm:text-sm text-red-400 transition-all duration-300 flex items-center gap-2 sm:gap-2.5 hover:bg-red-800 hover:pl-5 sm:hover:pl-6"
                      >
                        🗑️ Очистить историю
                      </button>
                    </li>
            ...
            <MediaFilesModal
                isOpen={showMediaFiles}
                onClose={closeMediaFiles}
                userId={otherUserId}
            />
